...
class TestTLSFlagsExtension(unittest.TestCase):
    ...
    def test_write_multiple_low_flags(self):
        ext = TLSFlagsExtension().create([0, 1, 5])

        self.assertEqual(bytearray([
            0, 0x3e,  # type (62)
            0, 2,  # ext length
            1, 0b00100011]),  # length prefix (1) + bits 0, 1 and 5
            ext.write())

    def test_write_multiple_flags(self):
        ext = TLSFlagsExtension().create([7, 8, 38])

        self.assertEqual(bytearray([
            0, 0x3e,  # type
            0, 6,  # ext length
            5,  # length prefix (5)
            0b10000000, 0b00000001, 0b0, 0b0, 0b01000000]),  # bits 7, 8 and 38
            ext.write())
    ...
    def test_parse_multi_byte(self):
        parser = Parser(bytearray([
            0, 0x3e,  # type
            0, 7,  # ext length
            6, 0b11000000, 0b0, 0b0, 0b0, 0b0, 0b00000100]))  # (6) + 6,7,42

        ext = TLSExtension().parse(parser)

        self.assertIsInstance(ext, TLSFlagsExtension)
        self.assertEqual(ext.flags, {6, 7, 42})
    ...
    def test_round_trip(self):
        ...
        self.assertEqual(ext2.flags, {0, 7, 8, 15, 2039})

    def test___repr__(self):
        ext = TLSFlagsExtension().create([5, 1, 0])

        self.assertEqual(repr(ext),
                         "TLSFlagsExtension(flags={0, 1, 5})")
